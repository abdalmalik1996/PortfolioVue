<template>
  <div class="skills__group">
    <div class="skills__list grid">
      <!-- html -->
      <div class="skills__data">
        <div class="skills__titles">
          <h3 class="skills__name">
            <font-awesome-icon icon="fa-brands fa-bootstrap" class="skills_icon bootstrap" />
            Bootstrap
          </h3>
          <span class="skills__number">50%</span>
        </div>
        <div class="skills__bar">
          <span class="skills__perecentage" style="width: 50%"></span>
        </div>
      </div>
      <div class="skills__data">
        <div class="skills__titles">
          <h3 class="skills__name"><img src="../../assets/icons/tailwind.svg" alt="" /></h3>
          <span class="skills__number">40%</span>
        </div>
        <div class="skills__bar">
          <span class="skills__perecentage" style="width: 40%"></span>
        </div>
      </div>
      <div class="skills__data">
        <div class="skills__titles">
          <h3 class="skills__name">
            <font-awesome-icon
              icon="fa-brands fa-github"
              class="skills_icon"
              style="color: #ffffff"
            />
            Github
          </h3>
          <span class="skills__number">60%</span>
        </div>
        <div class="skills__bar">
          <span class="skills__perecentage" style="width: 60%"></span>
        </div>
      </div>
      <div class="skills__data">
        <div class="skills__titles">
          <h3 class="skills__name">
            <img src="@/assets/icons/nodejs.svg" alt="" />
          </h3>
          <span class="skills__number">70%</span>
        </div>
        <div class="skills__bar">
          <span class="skills__perecentage" style="width: 70%"></span>
        </div>
      </div>
    </div>
  </div>
</template>
